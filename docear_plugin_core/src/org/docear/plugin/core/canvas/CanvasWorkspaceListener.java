...
public final class CanvasWorkspaceListener implements IWorkspaceNodeActionListener {

	public void handleAction(final WorkspaceActionEvent event) {
		final DocearCanvasController controller = DocearCanvasController.getController();
		if (controller == null) {
			return;
		}
		final File file = resolveFile(event);
		if (file == null || !DocearCanvasController.isCanvasFile(file)) {
			return;
		}
		if (!file.exists()) {
			event.consume();
		...
		controller.openCanvasFile(file);
		event.consume();
	}
