...
/**
 * Locates / injects a Java 8 JavaFX runtime (jfxrt.jar + native bin) so Draw.io
 * can embed when the active JVM is Adoptium/Temurin without JavaFX.
 */
final class DrawioJavaFxSupport {
	private static volatile Boolean available;
	private static volatile File resolvedJreHome;
	private static volatile String lastError = "";

	private DrawioJavaFxSupport() {
	}
	...
	static synchronized boolean ensureAvailable() {
		...
		if (canLoadJavaFxClasses()) {
			...
			resolvedJreHome = new File(System.getProperty("java.home", "."));
			return true;
		}
		...
		if (jre != null) {
			...
			try {
				...
				if (canLoadJavaFxClasses()) {
					available = Boolean.TRUE;
					LogUtils.info("Draw.io: loaded JavaFX from " + jre.getAbsolutePath());
					return true;
				}
				...
				LogUtils.warn("Draw.io: failed to inject JavaFX from " + jre, t);
			}
		}
		else {
	...
	static File getResolvedJreHome() {
		return resolvedJreHome;
	}
	...
	static List<String> candidateHints() {
		final List<String> hints = new ArrayList<String>();
		for (final File c : collectCandidateJreRoots()) {
			hints.add(c.getAbsolutePath() + (isJavaFxJre(c) ? " (OK)" : " (missing jfxrt)"));
		}
		return hints;
	}
	...
	private static List<File> collectCandidateJreRoots() {
		...
		for (int i = 0; i < libericaGlobs.length; i++) {
			...
			if (children != null) {
				for (int j = 0; j < children.length && j < 12; j++) {
	...
	static boolean isJavaFxJre(final File jreRoot) {
		if (jreRoot == null || !jreRoot.isDirectory()) {
			return false;
		}
		return jfxrtJar(jreRoot).isFile();
	}
	...
	private static File jfxrtJar(final File jreRoot) {
		...
		return new File(jreRoot, "jre/lib/ext/jfxrt.jar");
	}
	...
	private static void addJarToSystemClassLoader(final File jar) throws Exception {
		...
		// Fallback: child loader (may still fail for JFXPanel embed on some JREs)
