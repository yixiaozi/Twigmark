...
import org.freeplane.view.swing.features.reports.ReportDocumentView;
 ...
 * Reuses a single 「活动报表」tab when already open.
 */
public class UsageStatsReportService implements IExtension {
	...
	public void setReportVisible(final boolean visible) {
		if (visible) {
			...
			final ReportDocumentView existing = ReportDocumentService
			        .findByAssignmentKey(ReportDocumentService.KEY_ACTIVITY);
			final UsageStatsReportPanel panel;
			if (existing != null && !existing.isClosed()
			        && existing.getContent() instanceof UsageStatsReportPanel) {
				panel = (UsageStatsReportPanel) existing.getContent();
				lastPanel = panel;
				panel.refresh();
				ReportDocumentService.focusByKey(ReportDocumentService.KEY_ACTIVITY);
			}
			else {
				panel = new UsageStatsReportPanel();
				lastPanel = panel;
				...
				panel.refresh();
				ReportDocumentService.openOrFocus("活动报表", panel, ReportDocumentService.KEY_ACTIVITY);
			}
			...
			if (lastPanel != null) {
				...
				final ReportDocumentView existing = ReportDocumentService
				        .findByAssignmentKey(ReportDocumentService.KEY_ACTIVITY);
				if (existing != null) {
					ReportDocumentService.close(existing);
				}
				else {
